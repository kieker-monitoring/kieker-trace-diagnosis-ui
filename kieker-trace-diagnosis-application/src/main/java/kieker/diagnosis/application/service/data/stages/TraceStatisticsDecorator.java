 ...
 */
final class TraceStatisticsDecorator extends AbstractTransformation<Trace, Trace> {

	private final boolean ivPercentCalculationsRefersToTopMost;

	public TraceStatisticsDecorator( final boolean aPercentCalculationsRefersToTopMost ) {
		ivPercentCalculationsRefersToTopMost = aPercentCalculationsRefersToTopMost;
	}
	...
	private void addPercentValues( final OperationCall aCall, final long aParentDuration, final long aRootDuration ) {
		if ( aCall.getParent( ) == null ) {
			...
			if ( ivPercentCalculationsRefersToTopMost ) {
