 ...
 */
@Component
public class MonitoringStatisticsController extends AbstractController<MonitoringStatisticsView> {
	...
	protected void doInitialize( final boolean aFirstInitialization, final Optional<?> aParameter ) {
		if ( aFirstInitialization ) {
			...
			getView( ).getEndofmonitoring( ).textProperty( )
					.bind( Bindings.createStringBinding( ( ) -> assembleTimeString( endTimestamp.get( ) ), endTimestamp ) );
	...
	private String assembleSizeString( final File aFile ) {
		...
		final float size = calculateDirectorySize( aFile );
	...
	private String getNotAvailableString( ) {
		return getResourceBundle( ).getString( "notAvailable" );
	}

	private long calculateDirectorySize( final File aFile ) {
		return ( aFile.isFile( ) ) ? aFile.length( ) : Arrays.stream( aFile.listFiles( ) ).mapToLong( this::calculateDirectorySize ).sum( );
	}
