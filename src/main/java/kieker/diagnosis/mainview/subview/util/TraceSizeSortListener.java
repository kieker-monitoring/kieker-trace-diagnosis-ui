...
public final class TraceSizeSortListener extends AbstractTraceTreeColumnSortListener<AbstractTrace<?>> {
	...
	protected int compare(final AbstractTrace<?> fstTrace, final AbstractTrace<?> sndTrace) {
		final long fstSize = fstTrace.getRootOperationCall().getStackSize();
		final long sndSize = sndTrace.getRootOperationCall().getStackSize();

		return Long.compare(fstSize, sndSize);
	}
